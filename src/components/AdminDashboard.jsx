...
export default function AdminDashboard() {
  ...
  const catalogs = useMemo(() => {
    const doctors = Array.from(
      new Set(appts.map((a) => a.doctorName).filter(Boolean))
    ).sort();
    const specialties = Array.from(
      new Set(appts.map((a) => a.specialty).filter(Boolean))
    ).sort();
    const statuses = Array.from(
      new Set(appts.map((a) => a.status).filter(Boolean))
    );
  ...
  const matchesQ = (a) => {
    if (!q) return true;
    const t =
      `${a.patientName} ${a.doctorName} ${a.specialty} ${a.reason} ${a.status}`.toLowerCase();
  ...
  const saveSchedule = async (e) => {
    ...
    if (end <= start) {
      alert("Fin debe ser mayor a inicio.");
      return;
    }
    ...
    <div className="content content--admin">
      <section className="card hero hero--admin">
        <div>
          ...
          <p className="muted">
            Gestiona usuarios, doctores, especialidades y todas las citas.
          </p>
        </div>
                  ...
                  <tr key={a.id}>
                    ...
                    <td className="row-actions">
                      ...
                      <button
                        ...
                        onClick={() => updateStatus(a.id, "atendida")}
                      >
                        Marcar atendida
                      </button>
                    </td>
            ...
            <form className="form" onSubmit={saveSchedule}>
              <div className="form-row">
                <label>
                  ...
                  <select
                    required
                    value={schedForm.doctorId}
                    onChange={(e) =>
                      setSchedForm({ ...schedForm, doctorId: e.target.value })
                    }
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSchedForm({ ...schedForm, date: e.target.value })
                    }
                  />
                </label>
              </div>
              <div className="form-row">
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSchedForm({ ...schedForm, start: e.target.value })
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    value={schedForm.end}
                    onChange={(e) =>
                      setSchedForm({ ...schedForm, end: e.target.value })
                    }
                  />
                </label>
              </div>
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSchedForm({ ...schedForm, note: e.target.value })
                  }
